...
class ValidatePrescriptionView(View):
    ...
    def post(self, request):
        ...
        # Extract prescription_id from request body
        try:
            data = json.loads(request.body)
            prescription_id = data.get("prescription_id")
            if not prescription_id:
                return JsonResponse({"error": "Missing 'prescription_id' in request body"}, status=400)

            # Validate the prescription
            prescription = Prescription.objects.get(id=prescription_id)
            medical_treatments = MedicalTreatment.objects.filter(prescription=prescription)

            # Check if all medicines are filled
            all_medicines_filled = True
            for treatment in medical_treatments:
                if not treatment.medicine:
                    all_medicines_filled = False
                    break

            if all_medicines_filled:
                prescription.isValid = True
                prescription.save()
                return JsonResponse({"message": "Prescription validated successfully"}, status=200)
            else:
                return JsonResponse({"error": "Not all medicines are filled. Prescription not validated."}, status=400)
